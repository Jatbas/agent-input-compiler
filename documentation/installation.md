...
| `AIC-inject-conversation-id.cjs`  | `preToolUse`           | Injects `conversationId` into `aic_compile` and `aic_chat_summary` when present; for `aic_compile` also sets `editorId` to `cursor` on the augmented tool input. Requires a Cursor-native payload (`cursor_version`); see [§4.4](technical/cursor-integration-layer.md#44-runtime-boundary-guards-cursor_version). |
