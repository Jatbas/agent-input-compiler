...
| `AIC-require-aic-compile.cjs`     | `preToolUse`           | Compile gate: enforces `aic_compile` before other tools (per-generation marker, 300s default recency fallback, short sibling-race poll before deny — no strike limit). Override the window with `compileRecencyWindowSecs` in `aic.config.json`. Emergency bypass requires both `devMode` and `skipCompileGate` set to `true` in `aic.config.json`. Uses `failClosed: true`. See [§7.3](technical/cursor-integration-layer.md#73-pretooluse-unmatched--aic_compile-enforcement-gate). |
...
**Editor rule wording:** In Cursor, `.cursor/rules/AIC.mdc` lists each **show aic …** Bash line as one shell expression that runs `pnpm aic` when `aic.config.json` sets `"devMode": true` and otherwise `npx @jatbas/aic` (same shape as the published `TRIGGER_RULE_TEMPLATE` in `mcp/src/install-trigger-rule.ts`). In Claude Code, `.claude/CLAUDE.md` states the same routing with explicit `npx` and `pnpm` command lines under a short **Dev** note. The `npx` / `pnpm from the repo root when developing` wording in this section still describes the two binaries for human terminal use.

**run aic model test** — distinct from the five **show aic …** Bash flows above — is **MCP-only** (no matching CLI subcommand): the model calls `aic_model_test`, completes the challenges (including an `aic_compile` with a specific intent), calls `aic_model_test` again with answers, and presents the pass/fail result. Full agent wording for diagnostics and model test lives in the installed editor rules (`.cursor/rules/AIC.mdc`, `.cursor/rules/AIC-architect.mdc`, and `.claude/CLAUDE.md`).
...
| `last`                         | Same formatted "show aic last" table (**Cache**; **Guard (this run)**; **Top files** / **Excluded by** when the formatter includes them — see [Implementation spec — `aic_last`](implementation-spec.md#aic_last-mcp-tool)); use MCP `aic_last` for full JSON **`selection`**  |
| `chat-summary --project <dir>` | Per-conversation compilation stats for the given project directory                                                                                                                                                                                                             |
| `quality [--window <days>]`    | Per-project rolling-window metrics sourced from `quality_snapshots` (defaults to 7 days; max 365)                                                                                                                                                                              |
