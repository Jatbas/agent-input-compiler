...
When the Cursor installer has run, the project has **13** hook-related scripts deployed (**12** `AIC-*.cjs` event hooks plus `subagent-start-model-id.cjs`, which `AIC-subagent-compile.cjs` loads). They register like this:
...
| `AIC-require-aic-compile.cjs`     | `preToolUse`           | Compile gate: enforces `aic_compile` before other tools (per-generation marker, 120s recency fallback, deny-count cap). Emergency bypass requires both `devMode` and `skipCompileGate` set to `true` in `aic.config.json`. Uses `failClosed: true`. See [§7.3](technical/cursor-integration-layer.md). |
...
End users rarely need these; integrators and contributors use them for uninstall, CI, or repo development. The last row documents a JSON field in `aic.config.json`, not a process environment variable.
...
| `skipCompileGate` in `aic.config.json` | Cursor `preToolUse` gate hook; Claude Code compile helper                                | Emergency bypass: when `true` **and** `devMode` is also `true`, skips the Cursor compile gate and Claude Code hook-driven compilation. Remove immediately after fixing the issue that required it.                                             |
...
4. Appends missing lines to `.gitignore`, `.eslintignore`, and `.prettierignore` using the same manifest as `shared/src/storage/aic-ignore-entries.json`: `.aic/`, `aic.config.json`, `.cursor/rules/AIC.mdc`, `.cursor/hooks.json`, `.cursor/hooks/AIC-*.cjs`, and `!.cursor/hooks/aic-dir.cjs` (same list for all three files; existing entries are left unchanged)
...
- In `<project>/.cursor/`: removes AIC hook entries in `hooks.json`, deployed `AIC-*.cjs` scripts and `subagent-start-model-id.cjs` under `hooks/`, and the trigger rule `rules/AIC.mdc`.
