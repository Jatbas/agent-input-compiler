...
| KL-006 | Telemetry | **Partially addressed:** `compilation_log.model_id` is now populated when `ANTHROPIC_MODEL` (Claude Code) or `CURSOR_MODEL` (Cursor) env vars are set. Implemented via `ModelDetector` interface, `ModelEnvHints`, and `ModelDetectorDispatch` adapter; compile-handler uses `getModelId(editorId)` as fallback when the client omits `modelId`. Full project-plan design (EditorAdapter, CursorAdapter/ClaudeCodeAdapter/GenericMcpAdapter, EditorAdapterRegistry, file-based detection e.g. `~/.cursor/settings.json`) remains future work. Related: GAP-06 in `gaps.md`. | Phase 1 |
