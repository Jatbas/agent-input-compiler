...
| W14: Release 0.7.0                              | Pending | all                                          | W01-W13       | Version 0.7.0; changelog; tag, push, CI, GitHub release                                          |
...
**Components:** Task 177 Installation and integration docs symmetry, Task 178 Audit and fix installation.md, Task 179 compilation_log editor_id and conversation_id + doc alignment, Task 181 EditorId attribution fix, Task 183 Claude Code global-only installation
**Completed:**
...
- U06 (Task 174): Updated documentation/installation.md: Claude Code ToC (Plugin, Direct Installer, Prerequisite, Troubleshooting), Server Scope row (plugin or project config), Per-Project Artifacts (added .claude/settings.local.json row, note on plugin vs direct installer paths), Version Updates (plugin and direct installer bullets), full Claude Code section (Plugin recommended, Direct Installer for development, Prerequisite npx @jatbas/aic-mcp, Trigger Rule, Hooks table, Hook Lifecycle, How Hooks Are Delivered for both paths, Troubleshooting hooks not firing). Writing-quality fixes: "in this project" → "in the AIC repo", Prerequisite note for @jatbas/aic. Lint, typecheck pass.
- U06 (Task 174, Recommended scope): Reordered installation.md so Claude Code section follows Cursor (ToC–body match). Removed Phase T/U/W from What Gets Published, Per-Project Artifacts table, database line, Server Scope table; Claude Code intro capability-based; Direct Installer "first-compile bootstrap"; How Hooks Are Delivered shortened with link to design doc. Other Editors package set to @jatbas/aic@latest. Factual fix: installCursorHooks → Cursor installer (integrations/cursor/install.cjs). Writing fix: W10 → The server. Verification: writing-quality, factual-accuracy, cross-doc, reader-simulation subagents; lint, typecheck pass.
...
- T08 (Task 166): Created `integrations/claude/hooks/aic-block-no-verify.cjs` (PreToolUse Bash). stripQuoted, readStdinSync; run(stdinStr) parses JSON, command from tool_input?.command ?? input?.input?.tool_input?.command; deny when isGitCmd && hasNoVerify (--no-verify or \s-n\b); hookSpecificOutput with permissionDecision "deny", permissionDecisionReason; "{}" otherwise/on throw. Created `integrations/claude/__tests__/aic-block-no-verify.test.cjs` (deny_git_no_verify, deny_git_short_n, allow_git_without_flag, allow_non_git, allow_quoted_no_verify_in_message, allow_empty_or_malformed). Lint, typecheck, test, knip, lint:clones 0.
...
- T09 (Task 164): Created `integrations/claude/hooks/aic-session-end.cjs` (SessionEnd hook). Parses stdin (session_id, reason, cwd with input fallback); ensures `.aic` with mode 0o700; appends one JSON line to `prompt-log.jsonl` ({ sessionId, reason, timestamp }); unlinks `.aic/.session-context-injected` and `os.tmpdir()/aic-cc-edited-<sessionId>.json` (ignore ENOENT). All I/O in try/catch; exit 0 always. Exports run(stdinStr) for T11. Lint, typecheck, test, knip, lint:clones 0.
- T06 (Task 163): Created `integrations/claude/hooks/aic-after-file-edit-tracker.cjs` (PostToolUse Edit|Write). Reads JSON stdin (session_id, tool_input.path with input fallback); no file I/O when path missing; sanitizes session_id for temp filename; appends resolved path to `os.tmpdir()/aic-cc-edited-<sanitized>.json` (create/read/merge/dedupe/write); returns "{}". readStdinSync + run(stdinStr) export; CLI writes run(raw) to stdout, exit 0. Created `integrations/claude/__tests__/aic-after-file-edit-tracker.test.cjs` (temp_file_created_on_first_invocation, temp_file_appended_avoid_duplicate, output_empty_json, missing_path_no_op, session_id_sanitized). Lint, typecheck, test, knip, lint:clones 0.
