...
Single global MCP server registered in `~/.cursor/mcp.json`. One server process handles all workspace folders. Each `aic_compile` call specifies its `projectRoot`; the server creates or reuses a `ProjectScope` per project via a `ScopeRegistry`. The database is global at `~/.aic/aic.sqlite` with a `project_root` column on per-project tables. Per-project files (`aic.config.json`, `.aic/project-id`, `.cursor/rules/AIC.mdc`, `.cursor/hooks/`) remain in each project directory. A stable UUIDv7 project ID in `.aic/project-id` survives folder renames. Strict duplicate prevention ensures at most one AIC server instance per workspace. See `implementation-spec.md` §Phase W for full schema SQL, store changes, and migration strategy.

| Component                                               | Status  | Package                                   | Deps   | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------- | ------- | ----------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| W1: Cross-platform path normalisation                   | Pending | shared/src/core/types/paths.ts            | —      | Add `normaliseProjectRoot(raw: string): AbsolutePath` to `paths.ts`. Resolves to absolute, strips trailing separator, lowercases Windows drive letter. All DB writes and Map keys go through this function. See impl-spec §W1.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| W2: Global database migration (011-global-project-root) | Pending | shared/src/storage/migrations/ + mcp/src/ | W1     | Two parts: (A) Generic migration `011-global-project-root.ts` — adds `project_root TEXT NOT NULL DEFAULT ''` + index to 7 tables (`compilation_log`, `cache_metadata`, `tool_invocation_log`, `session_state`, `file_transform_cache`, `config_history`, `telemetry_events`); adds `projects` table (`project_id TEXT PK`, `project_root TEXT UNIQUE`, `created_at`, `last_seen_at`). Migration is schema-only, uses `DEFAULT ''` so it runs on existing rows without data loss. (B) Data backfill in `mcp/src/server.ts` startup — after migration runs, if any rows have `project_root = ''`, backfill them with the current `projectRoot` value. This preserves all existing compilation history, cache, telemetry, and session data. DB location moves from `{projectRoot}/.aic/aic.sqlite` to `~/.aic/aic.sqlite` by copying the existing file on first startup if the global DB does not yet exist. See impl-spec §W2. |
| W3: Stable project ID for folder rename detection       | Pending | shared/src/storage/ + mcp/src/            | W2     | Generate UUIDv7 in `{projectRoot}/.aic/project-id` on first `ensureProjectInit`. On each `aic_compile`: read project-id, look up in `projects` table. If path changed, update `project_root` in `projects` and all 7 per-project tables via `UPDATE ... SET project_root = ? WHERE project_root = ?`. See impl-spec §W3.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| W4: Lazy project scope (ScopeRegistry)                  | Pending | shared/src/storage/ + mcp/src/server.ts   | W2, W3 | `ScopeRegistry` interface with `getOrCreate(projectRoot): ProjectScope` and `close(): void`. Backed by `Map<AbsolutePath, ProjectScope>` (normalised key). `main()` opens `~/.aic/aic.sqlite` once, passes global `ExecutableDb` to `createMcpServer`. All store constructors gain `projectRoot` param; all SQL queries add `WHERE project_root = ?`. Compile handler calls `scopeRegistry.getOrCreate(args.projectRoot)`. `CompilationRunner` and `createFullPipelineDeps` instantiated per project. See impl-spec §W4.                                                                                                                                                                                                                                                                                                                                                                                                     |
| W5: Per-folder disable via `aic.config.json`            | Pending | mcp/src/handlers/ + shared/src/config/    | W4     | Add `"enabled": boolean` to config schema (default `true`, omitted = enabled). When `false`: compile handler returns early with user-facing message, no DB writes, `show aic status` shows "Disabled". Config type in `shared/src/config/` and Zod schema in `mcp/src/schemas/` both updated. See impl-spec §W5.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| W6: Strict duplicate prevention (global vs workspace)   | Pending | mcp/src/server.ts                         | —      | Upgrade `detectInstallScope` check from warning to hard stop. If scope is `"both"`: log error to stderr explaining AIC must be in one location only, then `process.exit(1)`. `"global"` and `"workspace"` both proceed normally. AIC dev workspace uses `"workspace"` scope and is unaffected. See impl-spec §W6.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| W7: AIC commands scoped by project                      | Pending | mcp/src/server.ts                         | W4     | `aic://status` aggregates globally (all projects) with per-project breakdown. `aic://last` filters by most-recent `projectRoot` from conversation. `aic_chat_summary` adds `projectRoot` to response. New `aic_projects` MCP tool lists all projects from `projects` table (project ID, path, last seen, compilation count). `SqliteStatusStore` gains project-scoped query methods. See impl-spec §W7.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| W8: Documentation update for global server architecture | Pending | documentation/                            | W4     | Update all docs that reference per-project isolation or contradict the global DB model. Files: `implementation-spec.md` §8 (remove "No shared state, no global database" — replace with global DB + per-project isolation via `project_root` column), `implementation-spec.md` §8b startup sequence (update to show global DB open + scope registry), `project-plan.md` §multi-project table and §non-goals (update "Cross-project shared database" non-goal — now a feature with column-level isolation), `security.md` §local-first table (update "no shared state" to "single local DB, project-level column isolation"), `documentation-review.md` DR-09 (cross-project view now possible via `aic_projects` tool), `.cursor/rules/AIC-architect.mdc` (update any DB/storage rules referencing per-project DBs). See impl-spec §W8.                                                                                      |

Notes:

- **Dependency order:** W1 → W2 → W3 → W4 → W5/W7/W8. W6 is independent and can be done in parallel with W2–W5. W8 (docs update) depends on W4 so code references are accurate.
...
- **Data preservation (mandatory):** The existing database at `{projectRoot}/.aic/aic.sqlite` contains all historical compilation, cache, telemetry, and session data. On first startup after upgrade: if `~/.aic/aic.sqlite` does not exist, **copy** (not move) the existing per-project DB to `~/.aic/aic.sqlite`, then run migration 011. The migration is generic (schema-only, `DEFAULT ''`); a separate backfill step in the composition root updates all rows where `project_root = ''` to the current `projectRoot`. This ensures zero data loss.
- **Store constructor changes:** Every SQLite store class gains `projectRoot: AbsolutePath` as a constructor parameter. All INSERT statements include `project_root`. All SELECT/UPDATE/DELETE statements add `WHERE project_root = ?`. This is the largest change surface — 10+ store files.
- **Folder rename detection:** On `aic_compile`, read `.aic/project-id`. If the UUID maps to a different `project_root` in the DB, the folder was renamed — run `UPDATE` across all per-project tables. If UUID not in DB, it's a new project — insert into `projects`. Works across macOS, Windows, Linux via `normaliseProjectRoot`.
- **AIC dev workspace:** Uses workspace-scoped MCP (`tsx mcp/src/server.ts`). Hook bypass (0.5.4) handles the preToolUse chicken-and-egg. No conflict with global installs in other projects.
