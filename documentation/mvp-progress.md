...
| KL-006 | Telemetry | `compilation_log.model_id` is never populated. The project plan describes an `EditorAdapter` interface with `detectModel(clientInfo)` that would auto-detect the active model from MCP client info or editor config files (e.g. `~/.cursor/settings.json`). This architecture is designed (project plan §2.2, §2.3) but not implemented. Current code passes `null` from the schema default, which the pipeline converts to empty string → `NULL` in SQLite. Implementing requires: `EditorAdapter` interface, `CursorAdapter`/`ClaudeCodeAdapter`/`GenericMcpAdapter` implementations, `EditorAdapterRegistry`, and wiring in the MCP composition root. Related: GAP-06 in `gaps.md`. | Phase 1 |
...
**Components:** PythonProvider (AST-safe)
**Completed:**
