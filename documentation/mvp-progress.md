...
| W02: Schema migration 011 (columns only)        | Pending | shared/src/storage/migrations/         | W01           | New migration file `shared/src/storage/migrations/011-global-project-root.ts`. Adds `project_root TEXT NOT NULL DEFAULT ''` column + index to 8 tables: `compilation_log`, `cache_metadata`, `tool_invocation_log`, `session_state`, `file_transform_cache`, `config_history`, `telemetry_events`, `guard_findings`. Adds `projects` table (`project_id TEXT PK`, `project_root TEXT UNIQUE NOT NULL`, `created_at TEXT NOT NULL`, `last_seen_at TEXT NOT NULL`). Schema-only DDL — no runtime backfill code in the codebase. Existing rows get `project_root = ''` via DEFAULT. Register the migration in `open-database.ts` migration array. DB stays at `{projectRoot}/.aic/aic.sqlite`. See impl-spec §W2.                                                                                                                                                                                                                                                                                                                                                          |
...
**Components:** W01 Cross-platform path normalisation (Task 121)
**Completed:**
