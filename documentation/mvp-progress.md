...
Context quality and compilation performance beyond the base heuristic pipeline. Delivered: symbol-level intent matching (ContextBench 2026), bidirectional import graph with reverse dependency walking (InlineCoder/RIG 2026 — 12.2% accuracy gain), adaptive scoring weights per task class, structural context map (RIG 2026 — 57.8% efficiency gain), granular file-level transformation cache, async parallel file I/O, and fast-glob double-stat elimination. Remaining: chunk-level file inclusion (SWE-Pruner 2026 — 23-54% token reduction via task-aware line-level pruning) and cached RepoMap with file watcher for near-zero subsequent scan latency. All implemented within AIC's hexagonal architecture via new classes/interfaces (OCP).
...
| CL06: Fix dev-mode bypass                               | Done   | integrations/cursor/              | CL01      | Bypass gate via AIC_DEV_MODE=1 instead of mcp/ path detection                                     |
...
| Multi-repo benchmark suite (multi-scale datapoints)                  | Pending | test/                      | —              | Token reduction at multiple project scales                                                                                                                                                                                              |
| Comparative benchmarks vs. native editor context                     | Pending | test/                      | —              | AIC vs native editor context selection                                                                                                                                                                                                  |
| Visual demo (GIF/recording) in README                                | Pending | ./                         | Phase N–U      | Screen recording of AIC in editor                                                                                                                                                                                                       |
| Cursor integration layer document                                    | Done    | documentation/             | —              | `documentation/cursor-integration-layer.md` — reference doc covering all Cursor hook events, adapter pattern, known limitations, capabilities table; Phase CL isolation tasks tracked in mvp-progress.md                                |
| Claude Code integration layer document                               | Done    | documentation/             | —              | `documentation/claude-code-integration-layer.md` — reference doc covering all Claude Code hook events, adapter pattern, output format rules, known bugs; Phase T/U implementation tasks tracked in mvp-progress.md                      |
| Documentation audit for public release                               | Pending | documentation/             | Cursor+CC docs | Remove internal artifacts (tasks/done/), rewrite mvp-progress as roadmap, review future/ docs language, trim integration layer content from `implementation-spec.md` and replace with references to the two integration layer documents |

> **Integration layer doc standard:** Final versions must be fully technical — matching the depth and structure of `implementation-spec.md` (exact field names, schemas, data flows, architectural decision rationale). Only status language is removed from the `future/` drafts (phase labels, legacy status labels, unfinished-work checklists). Technical depth is preserved in full. Once both documents exist, `implementation-spec.md` will have its integration layer sections trimmed and replaced with references to these documents.
...
| AE03: Sanitise cache values before pipeline use    | Done   | mcp/src/schemas/ + mcp/src/handlers/         | AE02 | On the server side, pass cache-derived `modelId`/`conversationId`/`editorId` through the same Zod schema constraints used in `compilation-request.ts` before using them in SQL or tool responses      |
| AE04: Integration tests for malicious cache inputs | Done   | integrations/**tests**/ + mcp/src/**tests**/ | AE03 | Test vectors: overlong strings (>256 chars), control characters (`\x00`-`\x1f`), nested JSON objects as field values, empty strings, missing fields, duplicate keys; verify all are silently rejected |
...
| Component                                               | Status  | Package                                      | Deps | Description                                                                                                                                                                                       |
| ------------------------------------------------------- | ------- | -------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AF01: Document current model ID flow per editor         | Done    | documentation/                               | —    | Create a sequence diagram (text or mermaid) showing the full model capture → cache write → cache read → server resolve flow for each editor (Cursor, Claude Code, cursor-claude-code)             |
...
**Components:** Task 197 Config guard.allowPatterns → ContextGuard, Task 198 cache file audit (AE01), Task 201 integration tests malicious cache (AE04), Task 201 model ID flow doc (AF01)
**Completed:**
...
**Components:** Task 196 Cursor subagent compilation_log model_id, Task 199 strict field validation on cache reads (AE02), Task 200 sanitise cache values before pipeline use (AE03)
**Completed:**
