# Marker file simplification evaluation
...
- You merge or replace the two `.aic/` paths with a single artifact; treat that as a full re-read of this decision record, not a cosmetic edit.
...
This document answers whether `.aic/.session-start-lock` and `.aic/.session-context-injected` should be merged into a single file while preserving correct mutual exclusion for SessionStart, dual-path marker semantics for UserPromptSubmit, and cleanup on SessionEnd.

> It does not evaluate JSONL caches (`session-models.jsonl`, `prompt-log.jsonl`, `session-log.jsonl`) or edited-files temp storage under `os.tmpdir()`. For JSONL consolidation, see [JSONL cache unification evaluation](jsonl-cache-unification-evaluation.md).
...
## Assessment of a single marker file

**Two concerns:** The lock serializes concurrent SessionStart work. The marker records which `session_id` received SessionStart context for the UserPromptSubmit fallback. One path must still encode mutual exclusion and completed injection without ambiguity.

**Temp file plus rename:** POSIX states that when `rename` replaces an existing file name, the old file is removed ([POSIX `rename`](https://pubs.opengroup.org/onlinepubs/9699919799/functions/rename.html)). Same-directory replacement gives atomic visibility of the new content to readers that open the final path. A merged design needs explicit state transitions, crash behavior, and tests; prefer `rename` from a temp file in the same directory over in-place truncation when the body must appear atomically.

**SessionEnd ordering:** SessionEnd clears the marker then releases the lock. UserPromptSubmit reads the marker without holding the lock. A merged-file design must keep: after SessionEnd, `isSessionAlreadyInjected` is false until a new successful SessionStart writes state for the new session.

**Stale lock recovery after a merge:** Today recovery ties lock deletion to a non-empty marker. A merged artifact needs equivalent rules so a crash cannot leave the system blocked indefinitely.

## Recommendation

**Keep two paths under `.aic/` for the lock and the session-context marker** until a follow-up supplies a single-file state machine, atomic replacement, full interleaving tests, and a documented review of Windows behavior for the chosen primitives.

Treat this document as the decision record.

## Implementation prerequisites

If a later change merges the files:

- Preserve SessionStart mutual exclusion, UserPromptSubmit marker visibility, and SessionEnd cleanup ordering.
- Extend `integrations/shared/__tests__/session-markers.test.cjs` for every new transition and interleaving guarantee.
- Change `integrations/shared/session-markers.cjs` and `.cursor/hooks/session-markers.cjs` in the same commit per [Integrations shared modules reference](integrations-shared-modules.md).
