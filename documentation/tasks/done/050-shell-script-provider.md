...
Implement the LanguageProvider interface for shell scripts (`.sh`, `.bash`) with regex for source/. (import-like) and regex-based L2 (function name) and L3. L1 returns [].

## Architecture Notes

- Regex only; no new dependency. Never throw (Null Object style).
- parseImports: `source "file"` or `. "file"`; return ImportRef[].
...
| Action | Path                                                                                            |
| ------ | ----------------------------------------------------------------------------------------------- |
| Create | `shared/src/adapters/shell-script-provider.ts`                                                  |
| Create | `shared/src/adapters/__tests__/shell-script-provider.test.ts`                                   |
...
```typescript
// shared/src/adapters/shell-script-provider.ts
...
export class ShellScriptProvider implements LanguageProvider {
  ...
  readonly extensions: readonly FileExtension[];
  constructor() {
    this.extensions = [toFileExtension(".sh"), toFileExtension(".bash")];
  }
...
| `SymbolType`    | `#core/types/enums.js` | `SYMBOL_TYPE.FUNCTION` |
| `SymbolKind`    | `#core/types/enums.js` | `SYMBOL_KIND.FUNCTION` |
...
### Step 1: Implement ShellScriptProvider

Create `shared/src/adapters/shell-script-provider.ts`. Constructor sets `id = "shell"` and `extensions = [toFileExtension(".sh"), toFileExtension(".bash")]`. parseImports: regex for `source\s+["']?([^"']+)["']?` and `\.\s+["']?([^"']+)["']?`; return ImportRef[] with source the captured path, symbols `[]`, isRelative when path starts with ".". extractSignaturesWithDocs: return `[]`. extractSignaturesOnly: regex for `function\s+(\w+)` or `(\w+)\s*\(\)\s*\{` (function name at start of line); build CodeChunk with symbolName, symbolType SYMBOL_TYPE.FUNCTION, filePath `toRelativePath("")`, startLine/endLine via `toLineNumber`, content the line, tokenCount `toTokenCount(0)`. extractNames: same regex for function names; return ExportedSymbol[] with SYMBOL_KIND.FUNCTION. Wrap all logic in try/catch; on error return []. Never throw.

**Verify:** `pnpm typecheck` passes; file imports only from #core.

### Step 2: Add tests

Create `shared/src/adapters/__tests__/shell-script-provider.test.ts`. Tests: parseImports with `source "x"` returns ImportRef[]; extractSignaturesOnly with `function foo` returns CodeChunk[]; extractNames returns ExportedSymbol[] for function names; empty or malformed content yields [] and does not throw.

**Verify:** `pnpm test shared/src/adapters/__tests__/shell-script-provider.test.ts` passes.

### Step 3a: Wire via composition root — `mcp/src/server.ts`

In `initLanguageProviders()`, import `ShellScriptProvider` and add a `sh` entry using the ternary-spread pattern. Check both `.sh` and `.bash` extensions:
...
Add `...sh` to the return spread. Adjust the spread list to match whatever providers exist at the time this task is executed.

**Verify:** `pnpm typecheck` passes.

### Step 3b: Wire via composition root — `cli/src/main.ts`

Apply the same change to `initLanguageProviders()` in `cli/src/main.ts`. Import `ShellScriptProvider` and add the `sh` ternary-spread entry with the same dual-extension check.
...
| ------------------------------------ | ----------------------------------------------- |
| parseImports_returns_refs            | source "x" produces ImportRef[]                 |
| extractSignaturesOnly_returns_chunks | function name produces CodeChunk[]              |
| extractNames_returns_symbols         | function names produce ExportedSymbol[]         |
| invalid_returns_empty                | Malformed content returns [] and does not throw |

## Acceptance Criteria

- [ ] ShellScriptProvider implements LanguageProvider for .sh, .bash
