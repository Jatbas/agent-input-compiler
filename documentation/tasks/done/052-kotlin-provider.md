...
Implement the LanguageProvider interface for Kotlin (`.kt`) with regex-based import parsing and regex-based L2/L3. L1 returns [].

## Architecture Notes

- Regex only; no new dependency. Never throw (Null Object style).
- parseImports: `import package.Class` or `import package.*`.
...
| Action | Path                                                                                            |
| ------ | ----------------------------------------------------------------------------------------------- |
| Create | `shared/src/adapters/kotlin-provider.ts`                                                        |
| Create | `shared/src/adapters/__tests__/kotlin-provider.test.ts`                                         |
...
```typescript
// shared/src/adapters/kotlin-provider.ts
...
export class KotlinProvider implements LanguageProvider {
  ...
  readonly extensions: readonly FileExtension[];
  constructor() {
    this.extensions = [toFileExtension(".kt")];
  }
...
### Step 1: Implement KotlinProvider

Create `shared/src/adapters/kotlin-provider.ts`. Constructor sets `id = "kotlin"` and `extensions = [toFileExtension(".kt")]`. parseImports: regex for `import\s+[\w.]+(?:\.[*])?`; capture the package path; return ImportRef[] with source the path, symbols `[]`, isRelative when source starts with ".". extractSignaturesWithDocs: return `[]`. extractSignaturesOnly: regex for `fun\s+(\w+)`, `class\s+(\w+)`, `object\s+(\w+)`; build CodeChunk[] with symbolName, symbolType SYMBOL_TYPE.CLASS for class/object and SYMBOL_TYPE.FUNCTION for fun, filePath `toRelativePath("")`, startLine/endLine via `toLineNumber`, content the line, tokenCount `toTokenCount(0)`. extractNames: same regex for public/top-level; return ExportedSymbol[] with SYMBOL_KIND.FUNCTION or SYMBOL_KIND.CLASS. Wrap all logic in try/catch; on error return []. Never throw.

**Verify:** `pnpm typecheck` passes; file imports only from #core.

### Step 2: Add tests

Create `shared/src/adapters/__tests__/kotlin-provider.test.ts`. Tests: parseImports with `import pkg.Class` returns ImportRef[]; extractSignaturesOnly with fun/class returns CodeChunk[]; extractNames returns ExportedSymbol[]; empty or malformed content yields [] and does not throw.

**Verify:** `pnpm test shared/src/adapters/__tests__/kotlin-provider.test.ts` passes.

### Step 3a: Wire via composition root — `mcp/src/server.ts`

In `initLanguageProviders()`, import `KotlinProvider` and add a `kt` entry using the ternary-spread pattern:

```typescript
const kt = projectHasExtension(projectRoot, ".kt") ? [new KotlinProvider()] : [];
```

Add `...kt` to the return spread. Adjust the spread list to match whatever providers exist at the time this task is executed.

**Verify:** `pnpm typecheck` passes.

### Step 3b: Wire via composition root — `cli/src/main.ts`

Apply the same change to `initLanguageProviders()` in `cli/src/main.ts`. Import `KotlinProvider` and add the `kt` ternary-spread entry.
...
| extractSignaturesOnly_returns_chunks | fun/class produce CodeChunk[]                   |
...
- [ ] KotlinProvider implements LanguageProvider for .kt
