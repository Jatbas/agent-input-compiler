...
Implement the LanguageProvider interface for HTML (`.html`) with regex for script src (import-like) and regex-based L2/L3. TypeScriptProvider already claims `.jsx`; this provider uses extensions [.html] only.

## Architecture Notes

- Regex only; no new dependency. Never throw (Null Object style).
- parseImports: `<script src="...">` and `<link rel="stylesheet" href="...">`; return ImportRef[] with source = URL or path.
...
| Action | Path                                                                                            |
| ------ | ----------------------------------------------------------------------------------------------- |
| Create | `shared/src/adapters/html-jsx-provider.ts`                                                      |
| Create | `shared/src/adapters/__tests__/html-jsx-provider.test.ts`                                       |
...
```typescript
// shared/src/adapters/html-jsx-provider.ts
...
export class HtmlJsxProvider implements LanguageProvider {
  readonly id = "html-jsx";
  readonly extensions: readonly FileExtension[];
  constructor() {
    this.extensions = [toFileExtension(".html")];
  }
...
| `SymbolType`    | `#core/types/enums.js` | `SYMBOL_TYPE.CLASS`    |
...
### Step 1: Implement HtmlJsxProvider

Create `shared/src/adapters/html-jsx-provider.ts`. Constructor sets `id = "html-jsx"` and `extensions = [toFileExtension(".html")]`. parseImports: regex for `<script\s+[^>]*src=["']([^"']+)["']` and `<link\s+[^>]*href=["']([^"']+)["']`; return ImportRef[] with source set to the captured URL/path, symbols `[]`, isRelative when source starts with "." or "/". extractSignaturesWithDocs: return `[]`. extractSignaturesOnly: regex for lines containing `<[a-zA-Z][a-zA-Z0-9]*` (opening tag); for each match build CodeChunk with symbolName the tag name, symbolType SYMBOL_TYPE.CLASS, filePath `toRelativePath("")`, startLine/endLine via `toLineNumber`, content the line, tokenCount `toTokenCount(0)`. If no matches, return []. extractNames: return `[]`. Wrap all logic in try/catch; on error return []. Never throw.

**Verify:** `pnpm typecheck` passes; file imports only from #core.

### Step 2: Add tests

Create `shared/src/adapters/__tests__/html-jsx-provider.test.ts`. Tests: parseImports with `<script src="x.js">` returns ImportRef[] with source "x.js"; extractSignaturesOnly returns CodeChunk[] when tag lines exist; extractNames returns []; empty or malformed content yields [] and does not throw.

**Verify:** `pnpm test shared/src/adapters/__tests__/html-jsx-provider.test.ts` passes.

### Step 3a: Wire via composition root — `mcp/src/server.ts`

In `initLanguageProviders()`, import `HtmlJsxProvider` and add an `html` entry using the ternary-spread pattern:

```typescript
const html = projectHasExtension(projectRoot, ".html") ? [new HtmlJsxProvider()] : [];
```

Add `...html` to the return spread. Adjust the spread list to match whatever providers exist at the time this task is executed.

**Verify:** `pnpm typecheck` passes.

### Step 3b: Wire via composition root — `cli/src/main.ts`

Apply the same change to `initLanguageProviders()` in `cli/src/main.ts`. Import `HtmlJsxProvider` and add the `html` ternary-spread entry.
...
| ------------------------------------ | ----------------------------------------------- |
| parseImports_returns_refs            | script src="x" produces ImportRef[]             |
| extractSignaturesOnly_returns_chunks | Tag lines produce CodeChunk[] when present      |
| extractNames_returns_empty           | extractNames returns []                         |
| invalid_returns_empty                | Malformed content returns [] and does not throw |

## Acceptance Criteria

- [ ] HtmlJsxProvider implements LanguageProvider for .html
