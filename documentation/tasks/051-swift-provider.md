...
Implement the LanguageProvider interface for Swift (`.swift`) with regex-based import parsing and regex-based L2/L3. L1 returns [].

## Architecture Notes

- Regex only; no new dependency. Never throw (Null Object style).
- parseImports: `import Module` or `import struct Module.Class`.
...
| Action | Path                                                                                            |
| ------ | ----------------------------------------------------------------------------------------------- |
| Create | `shared/src/adapters/swift-provider.ts`                                                         |
| Create | `shared/src/adapters/__tests__/swift-provider.test.ts`                                          |
...
```typescript
// shared/src/adapters/swift-provider.ts
...
export class SwiftProvider implements LanguageProvider {
  readonly id = "swift";
  readonly extensions: readonly FileExtension[];
  constructor() {
    this.extensions = [toFileExtension(".swift")];
  }
...
### Step 1: Implement SwiftProvider

Create `shared/src/adapters/swift-provider.ts`. Constructor sets `id = "swift"` and `extensions = [toFileExtension(".swift")]`. parseImports: regex for `import\s+[\w.]*` (module path after import); return ImportRef[] with source the module path, symbols `[]`, isRelative false (Swift imports are module names). extractSignaturesWithDocs: return `[]`. extractSignaturesOnly: regex for `func\s+(\w+)`, `class\s+(\w+)`, `struct\s+(\w+)`, `enum\s+(\w+)`; build CodeChunk[] with symbolName, symbolType SYMBOL_TYPE.CLASS for class/struct/enum and SYMBOL_TYPE.FUNCTION for func, filePath `toRelativePath("")`, startLine/endLine via `toLineNumber`, content the line, tokenCount `toTokenCount(0)`. extractNames: same regex; return ExportedSymbol[] with SYMBOL_KIND.FUNCTION or SYMBOL_KIND.CLASS. Wrap all logic in try/catch; on error return []. Never throw.

**Verify:** `pnpm typecheck` passes; file imports only from #core.

### Step 2: Add tests

Create `shared/src/adapters/__tests__/swift-provider.test.ts`. Tests: parseImports with `import X` returns ImportRef[]; extractSignaturesOnly with func/class returns CodeChunk[]; extractNames returns ExportedSymbol[]; empty or malformed content yields [] and does not throw.

**Verify:** `pnpm test shared/src/adapters/__tests__/swift-provider.test.ts` passes.

### Step 3a: Wire via composition root — `mcp/src/server.ts`

In `initLanguageProviders()`, import `SwiftProvider` and add a `swift` entry using the ternary-spread pattern:

```typescript
const swift = projectHasExtension(projectRoot, ".swift") ? [new SwiftProvider()] : [];
```

Add `...swift` to the return spread. Adjust the spread list to match whatever providers exist at the time this task is executed.

**Verify:** `pnpm typecheck` passes.

### Step 3b: Wire via composition root — `cli/src/main.ts`

Apply the same change to `initLanguageProviders()` in `cli/src/main.ts`. Import `SwiftProvider` and add the `swift` ternary-spread entry.
...
| ------------------------------------ | ----------------------------------------------- |
| parseImports_returns_refs            | import X produces ImportRef[]                   |
| extractSignaturesOnly_returns_chunks | func/class produce CodeChunk[]                  |
...
- [ ] SwiftProvider implements LanguageProvider for .swift
