...
| `aic-documentation-writer-team` | Delegatable | Experimental team variant: Track A (writing) and Track B (verification) run simultaneously; reconciliation replaces sequential write-then-review. Compare output against `aic-documentation-writer`.                      |
| `aic-researcher-team`           | Delegatable | Experimental team variant: Track A (codebase investigation) and Track B (adversarial challenge) run simultaneously; the challenger attacks framing before exploration completes. Compare output against `aic-researcher`. |
| `aic-roadmap-forge-team`        | Internal    | Experimental team variant: critics are embedded within tracks (Critic A in Track A, Critic B in Track B) rather than running as a separate post-investigation phase. Compare output against `aic-roadmap-forge`.          |
