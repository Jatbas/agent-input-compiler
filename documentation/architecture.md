...
The shipped server exposes two MCP tools whose names include `compile`; they are not interchangeable. The full eight-tool inventory also appears under [CLI Interface](#cli-interface) — MCP server mode.

**`aic_compile`** runs `runPipelineSteps`, the full repository-context compilation pipeline (see [Pipeline orchestration (`runPipelineSteps`)](implementation-spec.md#pipeline-orchestration-runpipelinesteps)). Editor hooks and the trigger rule invoke this tool. When `runPipelineSteps` completes, the handler returns JSON that includes `compiledPrompt`, `meta`, `conversationId`, `updateMessage`, and conditionally `configUpgraded` and `warnings` per [implementation-spec](implementation-spec.md). A narrow `subagent_stop` reparent path updates stored rows without running the pipeline; see [implementation-spec](implementation-spec.md).

**`aic_compile_spec`** is a separate MCP tool. Its handler validates the request with Zod, records invocations in `tool_invocation_log`, and returns `compiledSpec` plus `meta`. The returned `compiledSpec` string begins with `AIC aic_compile_spec: foundation stub; SpecificationCompiler not invoked.` Field-level response shape and stub semantics are defined in [implementation-spec](implementation-spec.md); the AIC Server tools table in [installation.md](installation.md#aic-server) lists the tool alongside its siblings.
...
- **CLI mode** — when invoked with `status`, `last`, `chat-summary`, or `projects` as the first argument, the process opens the database read-only, prints formatted table output to stdout, and exits. A separate `init` subcommand runs project bootstrap (writes local project artifacts; not a read-only diagnostic — see [installation.md](installation.md)). No MCP transport is started on these paths.

**MCP JSON vs CLI output:** MCP tools return structured JSON. The four diagnostic subcommands above print fixed-width tables from the same read-only store and omit MCP-only fields—for example `aic_last` may include `selection` (persisted selection trace) while `last` does not print it. `selection` is JSON `null` when no trace was stored (including when the latest compilation was served from the compilation cache). `aic_compile`'s `meta` does not carry that trace ([MCP response shape](implementation-spec.md#4-core-pipeline--implementation-detail)). For a fresh Steps 1–8 decision trace without writing through the compilation cache, use `aic_inspect` (`{ trace: … }`; shape differs from persisted `selection`) — [`aic_inspect` (MCP tool)](implementation-spec.md#aic_inspect-mcp-tool). Persisted trace semantics: [Selection trace (persistence and tools)](implementation-spec.md#selection-trace-persistence-and-tools).
