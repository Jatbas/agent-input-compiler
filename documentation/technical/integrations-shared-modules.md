...
| `compile-recency.cjs` | `RECENCY_WINDOW_MS`, `writeCompileRecency`, `isCompileRecent`, `writeTurnStart`, `writeTurnCompiled`, `isTurnCompiled`, `recencyFilePath`, `turnMarkerPath` | Project-scoped recency marker and per-conversation turn markers under `os.tmpdir()`; optional `compileRecencyWindowSecs` in `aic.config.json` overrides the default **300** second window. |
...
| `compile-recency.cjs` | `integrations/cursor/hooks/AIC-require-aic-compile.cjs`, `integrations/cursor/hooks/AIC-compile-context.cjs`, `integrations/cursor/hooks/AIC-before-submit-prewarm.cjs`, `integrations/cursor/hooks/AIC-subagent-compile.cjs`, `integrations/claude/hooks/aic-pre-tool-gate.cjs`, `integrations/claude/hooks/aic-prompt-compile.cjs`, `integrations/claude/hooks/aic-pre-compact.cjs`, `integrations/claude/hooks/aic-subagent-inject.cjs` |
