...
| `read-project-dev-mode.cjs` | `isDevModeTrue` | Reads `<project>/aic.config.json`; returns `true` only when `devMode === true` after `JSON.parse`; otherwise `false` (missing file or parse error). |
