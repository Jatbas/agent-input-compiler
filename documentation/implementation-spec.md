...
**Pre-step (before Step 1):** A **RepoMapSupplier** (`FileSystemRepoMapSupplier`, `WatchingRepoMapSupplier`) supplies the RepoMap. The project root is scanned with **`**/\*`** plus built-in **`DEFAULT_NEGATIVE_PATTERNS`** from **`shared/src/adapters/default-ignore-patterns.ts`** (including **`!.git/**`** and **`!**/.git/**`**), then paths are filtered with **`.gitignore`** semantics via the injected **`IgnoreProvider`** (**`file-system-repo-map-supplier.ts`**). Per-file token estimates use tiktoken, and the result is cached in the `repomap_cache` SQLite table. The heuristic selector consumes a **discovered** RepoMap (after intent-aware file discovery). Cache is re-used until the file-tree hash (SHA-256 of in-scope paths + sizes + mtimes) changes.
...
**Assembly vs budget (shipped):** Step 8 runs once per compile. Pre-step **overhead** does not token-count every rendered wrapper line from **`PromptAssembler`** (path headers, tier labels, reinforced-context blocks). The pipeline records **`promptTotal`** = `tokenCounter.countTokens(assembledPrompt)` on the returned string and does **not** re-run Step 6 when **`promptTotal` > totalBudget**.
...
**Downstream code budget:** `runPipelineSteps` subtracts measured pipeline overhead (structural map, session summary, spec ladder slice, constraints, fixed 100-token task header) from this **total** allocation to produce **codeBudget** for Step 4 and the main summarisation ladder. **`promptTotal`** on the pipeline result can exceed **totalBudget** when rendered Step 8 wrappers cost more than that static overhead estimate; there is no second summarisation pass to force **`promptTotal` ≤ totalBudget**.
...
**Output:** The compiled input (a single string, the final prompt). Token count of that string is **`promptTotal`** in `PipelineStepsResult`; it is not clamped to **totalBudget** by re-compressing when over.
