...
| `aic_compile_spec` | `mcp/src/schemas/compile-spec-request.schema.ts`  | `spec` required; optional `budget`                                                                                      | Stub response — **Stub body** below                                                                                          |

**`aic_compile_spec` stub body:** On validation success, `compiledSpec` is the multiline string built by `buildCompiledSpecStub` in `mcp/src/handlers/compile-spec-handler.ts`. The first line states that `SpecificationCompiler` is not invoked; following lines report type, code block, and prose counts, a budget segment (`none` or a number), and `totalTokensRaw`. `meta.totalTokensRaw` equals the sum of `estimatedTokens` across `spec.types`, `spec.codeBlocks`, and `spec.prose`. `meta.totalTokensCompiled`, `meta.reductionPct`, and `meta.transformTokensSaved` are `0`. `meta.typeTiers` maps keys `name + "\u0000" + path` (sorted) to `verbatim`, `signature-path`, or `path-only` from each type's `usage`. On validation failure the tool returns JSON `{ error: "Invalid aic_compile_spec request", code: "validation-error" }` as text content.
