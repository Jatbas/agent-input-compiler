...
| **Tool gate**           | **Cursor:** preToolUse enforces `aic_compile` before other tools via per-generation marker, 120s recency fallback, and deny-count cap (3 denials) unless the emergency bypass is active (both `devMode` and `skipCompileGate` true in `aic.config.json`). **Claude Code:** hooks invoke compilation on each user message; there is no Cursor-style enforcement gate. Text-only replies skip PreToolUse on both editors. See [cursor-integration-layer.md](technical/cursor-integration-layer.md) and [claude-code-integration-layer.md](technical/claude-code-integration-layer.md).                                                                                                    |
