...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| `aic_compile`                   | Zod validates `intent`, `projectRoot`, `modelId`, `editorId`, `configPath`, plus optional wire fields `triggerSource`, `conversationId`, `reparentFromConversationId`, `stepIndex`, `stepIntent`, `previousFiles`, `toolOutputs`, `conversationTokens` (see `mcp/src/schemas/compilation-request.ts`). `sessionId` is applied server-side from MCP session state when applicable ([§2.7](#27-agentic-workflow-support)) | `{ compiledPrompt: string, meta: CompilationMeta }`, or `{ reparented: true, rowsUpdated: number }` when `triggerSource` is `subagent_stop` with a reparent request | Primary — trigger rule or integration hooks |
...
The formula-derived `suggestedBudget` (see [Implementation specification — model-specific budget profiles](implementation-spec.md#model-specific-budget-profiles)) provides a sensible starting point but cannot account for project-specific characteristics. A 50-file microservice and a 5,000-file monorepo have very different context needs. The adaptive path uses `compilation_log` data — already collected in every compile — to close this gap:
...
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| **0 (baseline)** | Formula-derived budget from model context window. Single-shot compilation. Agentic editors can call `aic_compile` per-step with different `intent` strings — functional but not optimized.                       | Budget scales automatically with new models; agents work, just without session awareness |
