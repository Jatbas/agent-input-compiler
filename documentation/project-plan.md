...
| **Agentic session management**               | **Phase 1+** roadmap in this row: utilization-based budget **auto-tuning** beyond fixed headroom, a persistent cross-call cache for **`aic_compile_spec`**, and running the main **`ContentTransformerPipeline` / `SummarisationLadder`** on structured spec bodies. Shipped agentic slice today ([§2.7](#27-agentic-workflow-support)): optional **`aic_compile`** wire fields, **`session_state`** via **`SqliteAgenticSessionStore`**, session-aware cache keys, deterministic **`ConversationCompressor`** (last **10** steps in the header), **`SpecificationCompilerImpl`** behind **`aic_compile_spec`**, auto headroom + overhead-aware **codeBudget**. |
...
> **Session layer vs §2.7:** The Non-goals row lists **Phase 1+** items only: utilization-based **auto-tuning**, persistent **`aic_compile_spec`** cache, and main-pipeline transformer/ladder parity on spec inputs. [§2.7 Agentic workflow support](#27-agentic-workflow-support) describes the shipped agentic slice (**`AgenticSessionState`**, **`SpecificationCompilerImpl`**, effective-session cache rules, **`ConversationCompressor`**) — a different bundle than that deferred list.
...
| `SpecificationCompiler` / `SpecificationCompilerImpl`                                       | Compile structured `SpecificationInput` within a token budget → `SpecCompilationResult`; tier loop ships in `specification-compiler.ts` — main **`ContentTransformerPipeline` / `SummarisationLadder`** reuse on spec bodies and persistent **`aic_compile_spec`** cache remain roadmap ([§2.7](#27-agentic-workflow-support)) |
...
| **Chain of Responsibility** | Pipeline stages through prompt assembly (telemetry + deferred executor are outside `runPipelineSteps`)       | Each step processes its input and passes output to the next; a step can short-circuit (e.g. cache hit exits early)                                                               |
...
**Tokenizer / budget profile after resolution:** `ModelDetectorDispatch` uses `editorId`, optional wire `modelId`, and local editor settings (e.g. `~/.cursor/settings.json` → `model` for Cursor) so budget and tokenizer behaviour match the active editor. When detection still yields nothing, `GenericModelAdapter` is used — results are good, not maximally optimised. A formal `EditorAdapter` interface is Phase 2+.
...
Phase 2+ may introduce a worker-thread pool for expensive operations (e.g., VectorSelector embedding computation). If this happens, the `ContextSelector` interface allows swapping `HeuristicSelector` for a thread-safe `VectorSelector` — no core pipeline changes required (OCP). The single-threaded invariant applies to the baseline release and Phase 0.5 only.
...
**Shipped today — `BudgetAllocator` (Step 3):** Base budget resolves from rule-pack `budgetOverride`, then `contextBudget.perTaskClass`, then `contextBudget.maxTokens`. **Default `maxTokens` is `0` (auto).** When the resolved numeric base is **0**, the allocated **total** budget is the fixed headroom (no `min` with a separate cap):

```
headroom = max(0, 128_000 − 4_000 − conversationTokens − 500)
allocatedTotalBudget = headroom   // when base == 0 (auto)
```

When the resolved base is **positive** and `SessionBudgetContext.conversationTokens` is present — from the wire field or from the sum of `tokensCompiled` over all recorded session steps when `sessionId` and agentic session state exist (`deriveSessionContext` in `shared/src/core/run-pipeline-steps.ts`) — the allocated budget is the lesser of that base and the same headroom expression. When session context is omitted and the base is positive, the allocated budget is the base unchanged.

Constants match `CONTEXT_WINDOW_DEFAULT`, `RESERVED_RESPONSE_DEFAULT`, and `TEMPLATE_OVERHEAD_DEFAULT` in `shared/src/pipeline/budget-allocator.ts`; they are not read from model detection or per-model profile in shipped code.

**Shipped today — overhead-aware code path (`runPipelineSteps`):** After Step 3 yields **totalBudget**, the pipeline measures token overhead for the structural map, optional session summary text, spec-ladder slice (token count of compressed spec files, with ladder budget capped at **~20% of totalBudget**), merged constraint strings, and a fixed 100-token task header (`computeOverheadTokens` in `shared/src/core/run-pipeline-steps.ts`). **codeBudget** = max(0, totalBudget − overhead). `ContextSelector` and the main summarisation ladder operate on **codeBudget** so assembled prompts stay within the model window. **Dynamic maxFiles:** when `contextSelector.heuristic.maxFiles` is **0** (default), the effective cap is `ceil(sqrt(totalFiles))` clamped to **[5, 40]** (`resolveAutoMaxFiles`).

**Roadmap — richer model-derived budgeting:** Per-model `windowRatio` / `maxContextWindow × ratio` bases, Ollama `num_ctx` probing, and **automatic utilization-based tuning** beyond the fixed headroom auto mode are not implemented in shipped TypeScript; see [Implementation specification — Model-derived budgets](implementation-spec.md#model-derived-budgets-shipped-subset-and-roadmap). The `aic_status` tool already returns `budgetMaxTokens` and `budgetUtilizationPct` from existing rows; automated `windowRatio` suggestions remain a design target.
...
The **Analogous to** column describes conceptual similarity to main-compile ladder levels; the shipped `SpecificationCompilerImpl` path does not call `SummarisationLadder`.

**Shipped compression loop (`SpecificationCompilerImpl`):**

1. Assign initial tiers from `SpecTypeUsage` (deterministic, no scoring needed)
2. Deduplicate shared import lines across verbatim-tier bodies (`deduplicateImportsInText` / `splitLeadingImportsAndBody` in `specification-compiler.ts`)
3. Assemble `compiledSpec` from tier renderers; measure total tokens with the injected counter
4. While over budget, **demote** types: pick the lowest-priority remaining `verbatim` row (`constructs` before `calls-methods` before `implements`; larger measured fragment wins ties, then path, then name), then demote `signature-path` rows with the same tie-breaks
5. When types cannot be demoted further, drop the largest `codeBlocks` entry, then the largest `prose` entries, until within budget or no removable blocks remain
6. When every type is `path-only`, all code and prose blocks are removed, and the string still exceeds budget, append a fixed truncation banner (`WARN_TRUNCATION` in `specification-compiler.ts`)
...
- The same §2.7 type-tier vocabulary remains the contract for roadmap items: main-pipeline transformers/ladders on spec bodies, persistent `aic_compile_spec` cache, and tighter budget tooling

### Cache Keying for Sessions

The shipped compilation cache key is computed in `shared/src/pipeline/compilation-runner.ts` (`computeCompilationCacheKey` / `buildCacheKey`). The hashed preimage includes, in order: **task class** (from `IntentClassifier`), **project root**, **file-tree hash**, **config hash**, then—when both are present—**effective session id** (`conversationId` mapped to a session id when set, otherwise `sessionId`) and **`stepIndex`**, then—when non-empty—a **sorted** NUL-separated canonical string of paths from `toolOutputs.relatedFiles` (`canonicalRelatedPathsForSelectionCache` in `shared/src/pipeline/related-files-boost-context-selector.ts`). The same intent string can still miss cache if classification, tree, config, session inputs, or related paths differ. When session extensions and related-path suffixes are absent, behaviour matches the familiar single-shot pattern (classification outcome + tree + config). Authoritative step-by-step detail: [Implementation Spec](implementation-spec.md).

### Phasing

| Phase            | Agentic capability                                                                                                                                                                                                   | Notes                                                                                                                                                                                                                                                                                                                                  |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **0 (baseline)** | Config-based context budget (rule pack + `aic.config.json`). Single-shot compilation. Agentic editors can call `aic_compile` per-step with different `intent` strings — functional but not optimized.                | Default `maxTokens` **0** (auto headroom); default `maxFiles` **0** (auto cap); overhead-aware **codeBudget** in shipped TypeScript                                                                                                                                                                                                    |
| **0.5**          | `aic_status` tool surfaces aggregates and `budgetUtilizationPct` from `compilation_log`                                                                                                                              | Read-only metrics; no automated config recommendations in shipped MCP                                                                                                                                                                                                                                                                  |
| **1**            | Utilization-based **auto-tuning** beyond shipped fixed headroom; persistent cross-call cache for **`aic_compile_spec`**; wiring **`ContentTransformerPipeline` / `SummarisationLadder`** into structured spec bodies | Extended **`CompilationRequest`** + agentic wire fields, **`AgenticSessionState`** / **`session_state`**, **`ConversationCompressor`**, **`SpecificationCompilerImpl`** via **`aic_compile_spec`**, session-aware cache keys, auto headroom + overhead-aware **codeBudget**, dynamic **maxFiles** already ship — see Phase 0 **Notes** |
...
2. **Per-step compilation works**: If the agent calls `aic_compile` with a different intent each time, AIC compiles fresh context. The agent gets relevant files per step. Editors that omit **`conversationId`** lose the per-chat isolation that hooks expect for `session_state` and cache suffixes; the pipeline may still fall back to the MCP **`sessionId`** for the effective agentic id (`resolveEffectiveSessionId` in `shared/src/pipeline/compilation-runner.ts` — see also the long wire-field paragraph above and [§2.7 — Cache Keying for Sessions](#cache-keying-for-sessions)). A plain multi-call workflow still benefits from Context Guard and summarisation on every compile ([§2.7](#27-agentic-workflow-support)).
...
1. **Subagents start with fresh context (Cursor).** In Cursor, when an agent spawns a subagent (Task tool), the subagent starts its own conversation without AIC's session-start context. Cursor exposes `subagentStart` for gating only (no `additional_context`), so AIC cannot inject context at subagent spawn time. AIC registers `subagentStop` so `compilation_log` rows from the subagent session are reparented to the parent conversation for per-chat diagnostics (`aic_chat_summary`); that does not inject context into the subagent. **Note:** Claude Code provides a `SubagentStart` hook with `additionalContext` injection, which solves this limitation. See the integration layer comparison in [§2.2.1](#221-integration-layer--enforcement).
...
| **Task Class**           | A category assigned to the user's intent (e.g., `refactor`, `bugfix`, `feature`, `docs`, `test`). Determines which rule packs apply and which `contextBudget.perTaskClass` entry is consulted when present                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **Context Budget**       | The maximum token allowance for the compile **before** pipeline overhead is subtracted. Resolution order: rule-pack `budgetOverride`, then optional `contextBudget.perTaskClass`, then `contextBudget.maxTokens` (default **0** = auto headroom from fixed literals in `budget-allocator.ts`). Positive `maxTokens` is a manual cap; with `conversationTokens` and a positive base, Step 3 uses `min(base, headroom)`. Selection and the main ladder use **codeBudget** (total minus overhead — [§2.7](#27-agentic-workflow-support)). Per-model `windowRatio` and utilization **auto-tuning** beyond fixed headroom remain roadmap.                                             |
...
| **RepoMapSupplier**      | Supplies a `RepoMap` for the current project root: scans (respecting `.gitignore`), computes per-file token estimates via tiktoken, returns cached data when the file-tree hash is unchanged                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `contextSelector.heuristic.maxFiles`        | `0` (auto)                           | **`0`** = auto: effective cap is `ceil(sqrt(totalFiles))` clamped to **[5, 40]** (`resolveAutoMaxFiles` in `run-pipeline-steps.ts`). Positive values set a fixed cap.                                                                                                                                                                                                                                                                                                                                                                           |
...
Step 9 (Executor), when implemented, depends on a `ModelClient` abstraction, not on any SDK directly. This enforces OCP (new providers added without touching the Executor) and DIP (Executor never imports `openai` or `@anthropic-ai/sdk`).
...
| `session_state`           | Agentic session rows: composite PK `(session_id, project_id)`; `task_intent`, `steps_json`, `created_at`, `last_activity_at`. Written by `SqliteAgenticSessionStore` / `AgenticSessionState`. Schema in `001-consolidated-schema.ts`. Row expiry behaviour is product-level (see [§2.7](#27-agentic-workflow-support))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **1**   | OSS Release           | `1.0.0` | Public repo; docs site; CI/CD; npm package; `postinstall` team deployment; auto-detected dependency constraints; reverse dependency walking; optional cost estimation in `aic_status` (model-specific pricing, deferred from the baseline release since AIC is model-agnostic); **roadmap remainder:** per-model `windowRatio` / utilization **auto-tuning** beyond shipped auto headroom, persistent **`aic_compile_spec`** cache, main-pipeline **`ContentTransformerPipeline` / `SummarisationLadder`** on spec inputs. **Already shipping toward this phase:** extended **`CompilationRequest`** agentic fields, **`AgenticSessionState`** / **`session_state`**, **`ConversationCompressor`**, **`SpecificationCompilerImpl`** via **`aic_compile_spec`**, session-aware cache keys, overhead-aware **codeBudget** (see [§2.7](#27-agentic-workflow-support)) | 10+ external contributors; 100+ GitHub stars; stable API (no breaking changes for 4 weeks)        |
...
|                     | Vulnerability management      | ⚠️ `pnpm audit` in CI                                         | Baseline |
...
| Phase        | Milestone                                                                                                                                   |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **Baseline** | `security.md` published. `pnpm audit` in CI. Telemetry audit log. Privacy-by-default architecture.                                          |
...
The following metrics are intended to be derived from opt-in anonymous payloads (or equivalent aggregates from local SQLite exports) to evaluate system health and guide development:
...
2. **Identify Friction:** Find underperforming cohorts (e.g., "Exclusion rate for Python projects is only 15%").
