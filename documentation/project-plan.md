...
| `SpecificationCompiler` / `SpecificationCompilerImpl`                                       | Compile structured `SpecificationInput` within a token budget → `SpecCompilationResult`; tier loop and verbatim-only **`ContentTransformerPipeline` / `SummarisationLadder`** pre-pass ship in `specification-compiler.ts`; MCP handler persists **`aic_compile_spec`** outputs in SQLite **`spec_compile_cache`** via **`SqliteSpecCompileCacheStore`** — broader main-pipeline parity on all spec bodies beyond that verbatim-tier pre-pass remains roadmap ([§2.7](#27-agentic-workflow-support)) |
...
**Roadmap:** Extend the same `ContentTransformerPipeline` (Step 5.5) and `SummarisationLadder` (Step 6) wiring to spec content beyond the verbatim-only pre-pass (signature-path bodies, code blocks, prose) and converge behaviour with the full main-pipeline transform policy where applicable.
...
- The same §2.7 type-tier vocabulary remains the contract for roadmap items: broadening main-pipeline transform/ladder coverage on spec inputs beyond verbatim types and tighter budget tooling (the cross-call **`spec_compile_cache`** for **`aic_compile_spec`** is already shipped)
...
| **1**            | Utilization-based **auto-tuning** beyond shipped fixed headroom; extend **`ContentTransformerPipeline` / `SummarisationLadder`** integration on structured spec bodies beyond the shipped verbatim-only pre-pass                                                                                                                                                                                                                                                      | **Roadmap only** — utilization **auto-tuning** and broader spec-body ladder coverage in the previous column are not implemented in current TypeScript; the persistent **`aic_compile_spec`** cache (**`spec_compile_cache`**) ships in the Phase **0** baseline. Baseline remains Phase **0** / **0.5**.                                                                                                                                                                                    |
