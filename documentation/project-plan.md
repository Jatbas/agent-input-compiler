...
For **ignoring paths from scans vs from compiled context** (`.gitignore`, `aic-rules/<taskClass>.json` `excludePatterns`, `guard.allowPatterns`), see [§6 — Ignoring files and folders](#ignoring-files-and-folders).
...
**Example — explicit budget and selector caps** (fields validated by shipped `load-config-from-file.ts`; pattern-based context shaping lives in `aic-rules/` — [Ignoring files and folders](#ignoring-files-and-folders)):
...
> **Unimplemented options:** Some fields in the Field Reference below are planned but not yet implemented. Unimplemented options are ignored without error — you can set them today and they will take effect when the feature ships. If an option has no visible effect, it may still be unimplemented. **Shipped schema:** `shared/src/config/load-config-from-file.ts` does **not** read `contextSelector.heuristic.includePatterns`, `contextSelector.heuristic.excludePatterns`, or any top-level `excludePatterns` for context selection — use `aic-rules/<taskClass>.json` ([Ignoring files and folders](#ignoring-files-and-folders)). **Telemetry:** the minimal loader does not validate a `telemetry` object; local compile persistence is described in [§9.1](#91-compile-telemetry-local-sqlite).

### Ignoring files and folders

Use this when the question is: **“How do I stop AIC from using this folder in compiled context?”** Three different mechanisms apply at different pipeline stages; mixing them up is the usual source of confusion.

| Mechanism                                                                               | What it affects                                                                                                                                                                                       | What it does **not** do                                                                                                                                                    |
| --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Root `.gitignore`** (via `IgnoreProvider` in `shared/src/adapters/ignore-adapter.ts`) | **Repo-map scan:** ignored paths never appear in the `RepoMap` built by `FileSystemRepoMapSupplier`                                                                                                   | Does not change task-class constraints or token budgets                                                                                                                    |
| **`aic-rules/<taskClass>.json` → `includePatterns` / `excludePatterns`**                | **Selection:** only files still present in the `RepoMap` can be candidates; patterns filter or scope that set per merged rule pack (`IntentAwareFileDiscoverer`, then scoring in `HeuristicSelector`) | Does not remove paths from the filesystem scan; if a path is not ignored by scan rules, it stays in `RepoMap` until you add `.gitignore` / rely on built-in scan negatives |
| **`aic.config.json` → `guard.allowPatterns`**                                           | **Context Guard:** matching paths **skip content scanners** (secret / prompt-injection / command-injection style checks) after path-based `ExclusionScanner`; never-include globs still **block**     | Does **not** bypass never-include paths; does **not** remove files from the repo map or from heuristic selection                                                           |

**Scan stage (before rule packs):** The supplier glob is `**/*` plus **`DEFAULT_NEGATIVE_PATTERNS`** (`shared/src/adapters/default-ignore-patterns.ts`), then each candidate path must pass `.gitignore`. Many common trees (`**/node_modules/**`, `**/vendor/**`, `**/dist/**`, `**/tmp/**`, `**/log/**`, …) are already excluded at scan time; add **`.gitignore`** entries for project-specific generated or runtime directories you never want counted toward repo size or selection.

**Shipped `aic.config.json`:** There is **no** general `excludePatterns` (or `includePatterns`) key for context selection. Put glob lists in **`aic-rules/<taskClass>.json`** so they merge through `RulePackResolver`.

**Example — PHP / generated noise** (`aic-rules/general.json`). The same `excludePatterns` array can be repeated in `bugfix.json`, `feature.json`, `refactor.json`, `docs.json`, and `test.json` when you want identical pruning for every task class the classifier can emit.
...
Use **`includePatterns`** when you need a whitelist (one app inside a monorepo); otherwise **`excludePatterns`** alone is often enough to strip runtime and asset build output from selection.
...
| `contextSelector.heuristic.includePatterns` | —                                  | **Not in shipped Zod schema** — ignored if present. Use rule pack `includePatterns` in `aic-rules/<taskClass>.json` ([Ignoring files and folders](#ignoring-files-and-folders)).                                                                                                                                                                                                                                                                                                                                                                |
| `contextSelector.heuristic.excludePatterns` | —                                  | **Not in shipped Zod schema** — ignored if present. Use rule pack `excludePatterns` in `aic-rules/<taskClass>.json` ([Ignoring files and folders](#ignoring-files-and-folders)).                                                                                                                                                                                                                                                                                                                                                                |
...
1. **`.gitignore`** and built-in **`DEFAULT_NEGATIVE_PATTERNS`** shrink the supplier `RepoMap` (fewer paths scanned and token-estimated)
2. **`aic-rules/<taskClass>.json` → `includePatterns` / `excludePatterns`** narrow **selection** after the map is built — they do not change the glob that builds the map
...
**File-tree hash computation:** `SHA-256(sorted(relativePath + ":" + sizeBytes + ":" + mtimeMs) for each in-scope file)`. "In-scope" means the file appears in the **supplier** `RepoMap`: it matched the repo-map glob (`**/*` with `DEFAULT_NEGATIVE_PATTERNS`) and was accepted by `.gitignore` (`IgnoreProvider`). Rule-pack `includePatterns` / `excludePatterns` and `aic.config.json` **do not** change this set in the shipped pipeline — they apply later in `runPipelineSteps` (`IntentAwareFileDiscoverer` / selector). See [Ignoring files and folders](#ignoring-files-and-folders).
...
| New file added that passes repo-map scan filters          | Yes           | New path appears in the sorted path list                                                                                                     |
...
| `aic-rules/*.json` edited (no filesystem change)          | No            | Supplier `RepoMap` unchanged — `repomap_cache` keys off the serialized map (`compilation-runner.ts`, `serializeRepoMap`), not rule-pack JSON |
| `aic.config.json` changed                                 | No            | Does not alter paths returned by the repo-map supplier; file-tree hash unchanged unless config someday feeds scan filters (not shipped)      |
