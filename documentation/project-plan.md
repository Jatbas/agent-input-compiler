...
| **Agentic session management**               | Single-shot compilation in the baseline release; full session layer (tracker, deduplication, utilization-based **auto-tuning** of budgets beyond fixed headroom, `aic_compile_spec`) is Phase 1+; deterministic session header (`ConversationCompressor`) ships in the first agentic tranche — only the **last 10** recorded steps appear in the assembled prompt ([§2.7](#27-agentic-workflow-support)) |
...
| `aic_compile_spec` | `{ spec: SpecificationInput, budget?: TokenCount }` (see [§2.7](#27-agentic-workflow-support); Zod: `mcp/src/schemas/compile-spec-request.schema.ts`)                                                                                                                                                                                                                                                                   | MCP text JSON on success: `compiledSpec` foundation stub string and `meta` (`totalTokensRaw`, `totalTokensCompiled`, `reductionPct`, `typeTiers`, `transformTokensSaved`); invalid wire input returns JSON `{ error: string, code: "validation-error" }` in text content (`mcp/src/handlers/compile-spec-handler.ts`) | Foundation — stub response and `tool_invocation_log` until `SpecificationCompiler` ships |
...
The `aic_compile_spec` MCP tool is registered (see [§2.2](#22-mcp-server--primary-interface)). The shipped TypeScript handler validates input with Zod, records each invocation in `tool_invocation_log`, and returns deterministic foundation JSON: a `compiledSpec` stub string plus a `meta` object with `totalTokensRaw`, `totalTokensCompiled`, `reductionPct`, `typeTiers`, and `transformTokensSaved`. **`SpecificationCompiler` is not invoked** by this handler; the algorithm and types above describe the intended design once the compiler is integrated.

**What this enables (today):**

- A stable MCP wire shape and validation boundary for structured specification input (`CompileSpecRequestSchema`)
- Deterministic, logged responses so clients can integrate without rework when the full compiler ships
- The same §2.7 specification-compiler design remains the contract for future tiering, budget enforcement, and full `compiledSpec` output
...
| **1**            | Session Tracker + extended `CompilationRequest` + Specification Compiler (`aic_compile_spec`) + session-aware cache keying + utilization-based **auto-tuning** (beyond fixed headroom auto mode)      | Core agentic roadmap beyond Phase 0 (auto headroom, overhead **codeBudget**, dynamic **maxFiles**, and session clamp already ship — see Phase 0 **Notes**) |
...
2. **Per-step compilation works**: If the agent calls `aic_compile` with a different intent each time, AIC compiles fresh context. The agent gets relevant files per step. Without `conversationId` / session continuity, there is no session-aware deduplication, no derived `conversationTokens` for Step 3's clamp, and no MCP-visible agentic fields on the wire — even though a plain multi-call workflow still benefits from Context Guard and summarisation per compile ([§2.7](#27-agentic-workflow-support)).
...
| `aic_compile_spec` | Structured specification input (`CompileSpecRequestSchema`); **foundation stub** — validates wire input, records `tool_invocation_log`, returns stub `compiledSpec` and `meta` until `SpecificationCompiler` ships ([Implementation Spec §2](implementation-spec.md#2-what-ships), `mcp/src/handlers/compile-spec-handler.ts`) |
...
**Relation to `compilation_log`:** Task class, token totals, cache hit, duration, editor, model, and intent live on `compilation_log` (see [§20](#20-storage)). `telemetry_events` stores supplementary aggregates (`guard_findings`, `guard_blocks`, `transform_savings`, `tiers_json`, `repo_id`) keyed by `compilation_id`.
...
| **Never include keys in telemetry** | `telemetry_events` and `compilation_log` rows contain no API keys — typed numeric/enum fields only for metrics |
...
| Risk                               | Mitigation                                                                                                                                                                                                                      |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Telemetry leaks source code**    | `compilation_log` / `telemetry_events` store metrics and aggregates only (`intent` is stored on `compilation_log` for diagnostics — never file contents). See [§9.1](#91-compile-telemetry-local-sqlite) and [§20](#20-storage) |
...
| `session_state`           | Agentic session rows: composite PK `(session_id, project_id)`; `task_intent`, `steps_json`, `created_at`, `last_activity_at`. Written by `SqliteSessionStore` / `SessionTracker`. Schema in `001-consolidated-schema.ts`. Row expiry behaviour is product-level (see [§2.7](#27-agentic-workflow-support))                                                                                                                                                                                                                                                                                               |
...
| **1**   | OSS Release           | `1.0.0` | Public repo; docs site; CI/CD; npm package; `postinstall` team deployment; auto-detected dependency constraints; reverse dependency walking; optional cost estimation in `aic_status` (model-specific pricing, deferred from the baseline release since AIC is model-agnostic); **Session Tracker** + extended `CompilationRequest` agentic fields + **Specification Compiler** (`aic_compile_spec` MCP tool) + session-aware cache keying; **roadmap:** per-model `windowRatio` / utilization **auto-tuning** beyond shipped auto headroom + overhead-aware **codeBudget** (see [§2.7](#27-agentic-workflow-support)) | 10+ external contributors; 100+ GitHub stars; stable API (no breaking changes for 4 weeks)        |
