...
| `AdaptiveBudgetAllocator` _(Phase 1+)_                                                      | Adjust token budget for conversation length and project utilisation patterns → `number` (see [§2.7](#27-agentic-workflow-support))                                                                                              |
...
Each step needs _different_ context, but `aic_compile("refactor auth to use JWT")` returns the same compilation every time (cache hit). The agent's evolving needs — tool outputs, discovered errors, files it has already seen — do not reach **file selection** unless session state and forwarded fields are in play; even then, the selector does not consume `toolOutputs` today ([§2.7](#27-agentic-workflow-support)).

### Architectural Approach: Session Layer

The core pipeline through **prompt assembly** (conceptual Steps 1–8 in the implementation spec; handler telemetry is Step 10) remains unchanged when session fields are absent. A new **Session Layer** sits above the pipeline and manages multi-step state. This follows OCP: the pipeline is not modified; the session layer enriches the input before the pipeline runs. Components ship incrementally (from the first agentic tranche through later roadmap items — see Phasing below).
...
The `aic_compile` MCP tool Zod schema (`mcp/src/schemas/compilation-request.ts`) validates the same optional agentic fields as the internal `CompilationRequest` shape on the wire: `editorId`, `triggerSource`, `conversationId`, `stepIndex`, `stepIntent`, `previousFiles`, `toolOutputs`, and `conversationTokens`. The compile handler maps validated MCP arguments into `CompilationRequest`. Editors that omit these fields keep single-shot behaviour. When `conversationId` is present, it keys per-conversation session state (`session_state`), enabling deduplication, session-derived token totals for Step 3's `BudgetAllocator` clamp (when `conversationTokens` is omitted on the wire), and the deterministic session context header. Optional `toolOutputs` is forwarded on `PipelineStepsRequest` for each compile but `runPipelineSteps` does not read it for context selection (Steps 1–4): it does not change file scoring or boosting today. It is stored on the session step when the compile finishes and is summarised into the session header on **subsequent** compiles via `ConversationCompressor` (the header is built before the current step is recorded, so the current request's `toolOutputs` appear in the summary starting with the next compile in that session). `toolOutputs` is not part of the compilation cache key. Failure-aware boosting of files from tool-output content inside the selector remains future work.
...
- **Failure-aware selection (intended; not implemented)**: If `toolOutputs` contains test failures referencing `auth.test.ts`, the selector should boost that file and its imports regardless of the original intent scoring once selection steps read that signal (distinct from session-header summarisation of stored tool outputs).
...
### Budget allocation: shipped clamp vs roadmap

**Shipped today — `BudgetAllocator` (Step 3):** Base budget resolves from rule-pack `budgetOverride`, then `contextBudget.perTaskClass`, then `contextBudget.maxTokens` (default 8,000). When `SessionBudgetContext.conversationTokens` is present — from the wire field or from the sum of `tokensCompiled` over all recorded session steps when `sessionId` and agentic session state exist (`deriveSessionContext` in `shared/src/core/run-pipeline-steps.ts`) — the allocated budget is the lesser of that base and a **fixed** remaining-headroom estimate:

```
availableBudget = max(0, 128_000 − 4_000 − conversationTokens − 500)
allocatedBudget = min(baseBudget, availableBudget)
```

Constants match `CONTEXT_WINDOW_DEFAULT`, `RESERVED_RESPONSE_DEFAULT`, and `TEMPLATE_OVERHEAD_DEFAULT` in `shared/src/pipeline/budget-allocator.ts`; they are not read from model detection or `aic.config.json`. When session context is omitted, the allocated budget is the base budget unchanged. This clamp reduces AIC's context budget as recorded conversation tokens grow; the summarisation ladder then compresses more aggressively for a smaller budget.

**Roadmap — `AdaptiveBudgetAllocator` (Phase 1+ in the interface table):** Extends budgeting with **formula-derived base budgets** (`maxContextWindow × windowRatio`, floors/ceilings), optional **read-only utilization commentary** tied to `compilation_log`, and **automatic utilization-based tuning**. None of those paths are implemented in shipped TypeScript; see [Implementation specification — Planned model-derived budgets](implementation-spec.md#planned-model-derived-budgets). The `aic_status` tool already returns `budgetMaxTokens` and `budgetUtilizationPct` from existing rows; automated `windowRatio` suggestions and allocator-side auto-tuning remain design targets.

**Research validation (roadmap):** Difficulty-aware budgeting and feedback from execution data (SelfBudgeter, AdaCtrl, ACON) and "Lost in the Middle" motivate the Phase 1+ direction; the shipped clamp is a conservative fixed window slice, not the full adaptive design.
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| **0 (baseline)** | Config-based context budget (rule pack + `aic.config.json`). Single-shot compilation. Agentic editors can call `aic_compile` per-step with different `intent` strings — functional but not optimized.  | Default `maxTokens` 8,000; no formula-derived base budget in shipped TypeScript            |
...
| **1**            | Session Tracker + extended `CompilationRequest` + **AdaptiveBudgetAllocator** (formula-derived base + utilization learning) + Specification Compiler (`aic_compile_spec`) + session-aware cache keying | Core agentic roadmap; conversation-length **clamp** already ships inside `BudgetAllocator` |
...
| **Context Budget**       | The maximum token allowance for context included in the compiled input. Shipped resolution: rule-pack `budgetOverride`, then optional `contextBudget.perTaskClass`, then `contextBudget.maxTokens` (default 8,000). When `SessionBudgetContext.conversationTokens` is set, Step 3 clamps against fixed literals in `budget-allocator.ts` (see [§2.7](#27-agentic-workflow-support)). Formula-derived bases, `windowRatio`, and utilization auto-tuning are roadmap (`AdaptiveBudgetAllocator`, Phase 1+)                                                                                         |
...
| **1**   | OSS Release           | `1.0.0` | Public repo; docs site; CI/CD; npm package; `postinstall` team deployment; auto-detected dependency constraints; reverse dependency walking; optional cost estimation in `aic_status` (model-specific pricing, deferred from the baseline release since AIC is model-agnostic); **Session Tracker** + extended `CompilationRequest` agentic fields + **`AdaptiveBudgetAllocator` roadmap** (formula-derived base + utilization auto-tuning beyond today's `BudgetAllocator` session clamp) + **Specification Compiler** (`aic_compile_spec` MCP tool) + session-aware cache keying (see [§2.7](#27-agentic-workflow-support)) | 10+ external contributors; 100+ GitHub stars; stable API (no breaking changes for 4 weeks)        |
