...
| MCP-03 | Tool Poisoning                | N/A               | AIC exposes 7 tools (`aic_compile`, `aic_inspect`, `aic_projects`, `aic_status`, `aic_last`, `aic_model_test`, `aic_chat_summary`). No dynamic tool loading, no third-party tool registry, no remote tool discovery.                                                                                                                                                                                                                                                           | Fixed tool set compiled into the binary                                                                 |
...
Both Cursor and Claude Code require explicit user approval before MCP tools run. All seven AIC tools (`aic_compile`, `aic_inspect`, `aic_projects`, `aic_status`, `aic_last`, `aic_model_test`, `aic_chat_summary`) need approval where the editor enforces per-tool consent. **`aic_compile`** (and typically **`aic_inspect`**) must be allowed for full compilation and guard inspection.
