...
Track first-pass quality: for each dimension, record whether it was clean on first check or required a fix. This is informational — it helps calibrate the pre-write reference table and per-file quick checks over time but does not gate progress. Report the count in §5a (e.g. "18/18 first-pass clean" or "16/18 first-pass clean, fixed 2: readonly array in X, block comment in Y"). Dimensions 16–18 are conditional — exclude from the denominator when they don't apply (no transformer added, no migration created). If the per-file quick check (§3) already caught and fixed a violation during implementation, that dimension still counts as first-pass clean — the quick check is part of the first pass.
