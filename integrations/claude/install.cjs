...
- **Comments:** \`//\` style only — \`/* */\` and \`/** */\` are banned by ESLint; one line max enforced by ESLint. No JSDoc. **Delete-test:** mentally remove the comment and re-read the code — if the meaning is unchanged, the comment must be deleted. Only keep comments that explain a non-obvious *reason*: a cross-repo sync constraint, a non-obvious algorithmic choice, or a gotcha the code alone cannot express. Never describe what the code does (\`// Get the user\`, \`// Return result\`, \`// Loop over items\`).
