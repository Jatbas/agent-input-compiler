...
- **Mechanical gates:**
  ...
  - **Manifest:** the `planner-gate` record includes a structured `gates` array. C.5b/C.5c consume this manifest and do residual judgement review only; they do not rerun mechanical gate checks.
...
17. **Goal-to-acceptance-to-proof traceability.** Every atomic clause of `## Goal` must be covered by at least one task-specific `## Acceptance Criteria` bullet, and every task-specific acceptance bullet must name a concrete proof artifact the executor can verify after implementation: named test, command output, source symbol, schema/descriptor field, payload field, log line, migration/schema row, or file path. Generic invariants (`pnpm lint clean`) do not satisfy traceability. Mechanical check AM plus `validate-task.sh` acceptance-proof backstop.
