...
| **U**      | always                                           | Acceptance criteria are achievable: referenced tests exist and would pass, commands don't introduce failing patterns, `all tests pass` cross-refs TEST IMPACT, and every task-specific criterion names a proof artifact the executor can verify after implementation.   | Rewrite the criterion to reference a concrete artifact; add the affected test to the Files table.                                                                                                     |
...
| **AM**     | always                                           | Every atomic clause of `## Goal` is covered by at least one Acceptance Criteria bullet referencing a concrete proof artifact (test name, exported symbol, command output, schema/descriptor field, payload field, log-line, migration/schema row, file path, MCP tool). | Add the missing goal-traceability bullet and a concrete proof artifact; generic invariants (`pnpm lint clean`) do not satisfy this.                                                                   |
...
3. **Residual checks only** (report FOUND/NOT_FOUND, MATCH/MISMATCH, COMPATIBLE/INVALIDATED):
   ...
   - Acceptance criteria vs proof obligations: every goal clause has a concrete acceptance proof tied to a named test, command output, source symbol, schema/descriptor field, payload field, log line, migration/schema row, or file path. Generic toolchain success alone is not proof.
