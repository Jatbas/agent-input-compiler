...
6. **Sibling analysis + shared code prediction** (mandatory — all recipes):
   ...
   - **Renderer/formatter layout quorum (mandatory — target composes output strings):** When the target function is a renderer/formatter — composes CLI output, pretty-prints a report, emits markdown/HTML, or otherwise builds a human-readable string from data — structural features alone are not enough. SIBLING QUORUM must additionally catalog the LAYOUT PATTERN of every sibling renderer in the same file (or same output surface): (a) separator placement (where `SEP` / divider lines sit relative to the hero/title block and footer), (b) hero/title positioning (whether a one-line summary sentence appears and where — before, between, or after separators), (c) column width convention (e.g. `const w = 30` across all renderers in the file), (d) footer structure (single-line footnote vs multi-line footnote constant vs inline), (e) row ordering (title, hero, metric rows, footer). Record each property per sibling. Compare the target function's intended output against this catalog row-by-row. If the target's layout would deviate from the sibling majority, either realign to the majority or document the deviation in Architecture Notes under a `**Layout deviation:**` bullet with justification. Layout drift is not caught by any behavioral test and is the most common rendering-change oversight. Closes the drift observed in task 333 draft where the proposed hero line would have rendered as `Title\nHero\nSEP\nBody`, breaking the `[SEP, hero, SEP]` pattern used by every sibling renderer in `format-diagnostic-output.ts`.
...
- LAYOUT PATTERN (conditional — target composes output strings: CLI renderer, pretty-printer, markdown/HTML emitter):
  - Separator placement: [e.g. `[SEP, hero, SEP, body, SEP, footer]` at formatStatusTable:242]
  - Hero/title positioning: [e.g. hero sits between two SEP lines; title above first SEP]
  - Column width: [e.g. `const w = 30` shared across every formatter in the file]
  - Footer structure: [e.g. multi-line footnote constant joined with `\n`]
  - Row ordering: [e.g. title → sep → hero → sep → metric rows → sep → footer]
  - Target function's intended layout: [AGREES with majority | DEVIATES — justified in Architecture Notes **Layout deviation:**]
- Or: SOLE SIBLING — only one sibling exists in layer, treated as canonical: [path]
- Or: NOT APPLICABLE — target is not a renderer/formatter (no string-composition output).
...
FIXTURE SIMULATION (conditional — from item 17c, runs whenever BEHAVIOR CHANGES has at least one entry; HARD RULE 25):
- Importers simulated: [full list of test files that import the modified symbol — unit, integration, benchmark]
- [test file]:[fixture name or line range] — fixture: [brief description of inputs] — new behavior: [pass-through observation] — CLASSIFICATION: UNCHANGED
- [test file]:[fixture name or line range] — fixture: [brief description of inputs] — new behavior: [observation] — CLASSIFICATION: ASSERTION FLIPS — delta: [old literal → new literal, or "remove X from expected set"]
- [baseline/golden path] — regeneration mechanism: [`vitest -u` | benchmark ratchet script | ...] — CLASSIFICATION: AUTO-RATCHET
- [test file]:[fixture name] — CLASSIFICATION: FIXTURE BLOCKED — latent bug: [description] — proposed resolution: [in-scope fix | deferred successor task NNN | justified workaround]
- Files table impact: [N "Modify" rows added for ASSERTION FLIPS fixtures]
- Architecture Notes impact: [M paths listed under `**Auto-ratcheting artifacts:**`]
- Or: No fixture simulation required — modifications are additive only.
