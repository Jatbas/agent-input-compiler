...
6. **Sibling analysis + shared code prediction** (mandatory — all recipes):
   ...
   - **Renderer/formatter layout quorum (mandatory — target composes output strings):** When the target function is a renderer/formatter (CLI output, pretty-printed report, markdown/HTML emitter, human-readable string from data), structural features alone are not enough. SIBLING QUORUM must additionally catalog the LAYOUT PATTERN of every sibling renderer in the same output surface: (a) separator placement (where `SEP`/divider lines sit relative to hero/title/footer), (b) hero/title positioning, (c) column width convention (e.g. `const w = 30`), (d) footer structure, (e) row ordering. Record per sibling. If the target's layout would deviate from the majority, either realign or document the deviation in Architecture Notes under `**Layout deviation:**`. Layout drift is not caught by behavioral tests — see `SKILL-drift-catalog.md §Sibling quorum — layout pattern` for the historical evidence.
...
17c. **Behavior-change fixture simulation** (mandatory when item 17 recorded at least one BEHAVIOR CHANGES entry — HARD RULE 25; consumes item 14d's TEST SURFACE SCAN). For every `BEHAVIOR CHANGES` entry:
...
- **Classify each fixture** into exactly one bucket:
  ...
  - `ASSERTION FLIPS` — the test file must be modified; record the exact delta (old literal → new literal, or "remove/add `path` in expected set"). Becomes a Files table Modify row.
  - `AUTO-RATCHET` — baseline/golden artifact regenerated via `vitest -u` or benchmark ratchet. Declare in `## Architecture Notes` under `**Auto-ratcheting artifacts:**`. Executor §5c Step 2 whitelist allows auto-staging only for declared paths.
