...
2. **Stage only touched files and commit in the worktree.**
   ...
   Before committing, run `git status --porcelain` and compare against the touched-files list. The **only** side-effects that may be silently staged are (a) lint-staged / prettier reformatting of files **already on the list**, (b) benchmark baseline files the task file explicitly marks as auto-ratcheting (search the task for "baseline", "ratchet", or "auto-update"), (c) generated artifacts named in a Config Changes or Build section of the task file, and (d) `test/benchmarks/baseline.json` **when the diff is a strict decrease** in an existing task key's `token_count` (the benchmark at `shared/src/integration/__tests__/token-reduction-benchmark.test.ts` calls `fs.writeFileSync` whenever `tokenCount < prev.token_count`, so this file ratchets deterministically on faster hardware on every `pnpm test` run — it is environment drift, not task scope). A baseline diff that adds a new task key, **increases** `token_count`, or is duration-only is **not** this case and remains a Blocked diagnostic (do not silently stage it — investigate). Anything else dirty in the worktree — test fixtures modified to pass new logic, integration snapshots you regenerated with `vitest -u`, sibling-test keyword adjustments, benchmark `expected-selection/*.json` that shrank because the new logic excludes more paths, adjacent refactors, or any file the task did not name — is a **Blocked diagnostic** per HARD RULE 9. Do not `git add` it. Do not rationalize it as a "legitimate side-effect". Stop and ask the user to extend scope, re-plan, or discard (see `SKILL.md` §Blocked Handling). Unrelated files → leave unstaged. Accidentally staged → `git reset HEAD <path>`.
