...
- **Scope discipline:** the Files table is a closed set. Out-of-list edits = Blocked diagnostic (HARD RULE 9). Legitimate side-effects are the narrow whitelist in `SKILL-phase-5-finalize.md §5c Step 3`.
...
- **Scope growth** — making a step pass requires editing a file that is not in the task's Files table and is not one of the enumerated legitimate side-effects in `SKILL-phase-5-finalize.md` §5c Step 3. Stop the moment you notice it; do not open the file.
...
- [ ] `§4b` dim 22 (goal-traceability acceptance) clean — every task-specific Acceptance Criteria bullet maps to a concrete proof artifact: named test, command output, source symbol, schema/descriptor field, payload field, log line, migration/schema row, or file path. Generic invariants alone do not satisfy this dim.
