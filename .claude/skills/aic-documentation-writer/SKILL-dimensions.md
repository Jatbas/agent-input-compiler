...
Grep the full document for: 'GAP', 'TODO', 'FIXME', 'will be added', 'future task'. For 'Phase [A-Z]' references: **first check the document regime** — if the target document is `documentation/project-plan.md` (a planning document), skip the phase-name grep entirely; phase names are structural sequencing content there, not stale markers. For all other documents, grep for 'Phase [A-Z]' and cross-reference against `documentation/tasks/progress/mvp-progress.md` (main workspace) to check if the referenced phase is complete while the text uses future tense. Report each marker with its location and whether it is actionable or informational.
