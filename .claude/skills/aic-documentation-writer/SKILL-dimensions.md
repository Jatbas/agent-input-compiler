...
Grep the full document for: 'GAP', 'TODO', 'FIXME', 'will be added', 'future task'. For phase heading references (`Phase (?:[A-Z]{1,2}|[0-9]+(?:\.[0-9]+)?)\b` — same pattern as Dimension 9 in `SKILL-standards.md`): **first check the document regime** — if the target document is `documentation/project-plan.md` (a planning document), skip the phase-name grep entirely; phase names are structural sequencing content there, not stale markers. For all other documents, grep for that pattern and cross-reference against `documentation/tasks/progress/aic-progress.md` (main workspace only — gitignored) to check if the referenced phase is complete while the text uses future tense. Report each finding with its location and whether it is actionable or informational.
...
- Note formatting: are supplementary asides (caveats, scope boundaries, clarifications, background context) formatted as blockquotes (`>`)? Flag: plain paragraphs that serve as notes but lack blockquote formatting; sequential notes that are separate blockquotes instead of a unified block (joined with `>` blank lines); sections with more than 1 note block; blockquotes that are actually labels or commands introducing a code block, table, or image (misclassified — should be bold text or subheading). See SKILL-standards.md §Notes and asides.
