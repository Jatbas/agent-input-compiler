...
- **Outputs:** Updated doc files or an audit report. The change specification and critic reports are scratch artifacts written to `.aic/runs/<run-id>/` and deleted on run-complete. Use `--keep-artifacts` if you need to inspect them after finalisation.
- **Non-skippable steps:** Classify mode → classify exposure + audit depth → Route → Explore (subagents) → Write Change Specification or Audit Report → Apply edits when approved → Critic round (editorial + factual + cross-doc always; reader if user-facing/mixed) → Audit-completeness critic (deep audit mode only, double-blind) → Finalise.
- **Mechanical gates:**
  `bash .claude/skills/shared/scripts/ambiguity-scan.sh <doc>` per touched public, prescriptive, planning, or executable task file; advisory only for internal working logs unless the user explicitly requests ambiguity cleanup.
...
1. **Dispatch the critics.** Editorial + factual + cross-doc are always dispatched. Reader is conditional (see rule 1a). Audit completeness (`critic-audit.md`) is dispatched in **deep audit mode only** (see Phase 3 §3b Critic 5). Every critic has its own prompt template in `prompts/`.
...
2. **Double-blind audit completeness.** In deep audit mode, `critic-audit.md` is dispatched without the explorer findings or other critics' reports — only the Structured Audit Report and the document. Keep it independent. Triage audits and internal working-doc audits skip Critic 5 unless the user explicitly requests a full audit.
...
- **Finalize gate failure.** Phase 4 `ambiguity-scan.sh` / `evidence-scan.sh` / HARD RULE 4 grep fails and the failure cannot be fixed by a bounded edit on the touched documents. Advisory ambiguity findings on internal working logs are reported, not blocking.
...
- Auditing docs for staleness, drift, gaps, or cross-document inconsistency; broad audits begin with exposure-aware triage before deep review.

## When NOT to use

- Code comments (use short inline `//` comments in the code).
- Creating task files (use `aic-task-planner`). Auditing or updating existing task/progress docs is allowed only when explicitly named, or as a light internal-doc pass inside a broad audit.
...
| --------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | ------------------- |
| 1. Analyze (classify mode + exposure/audit-depth route + explore via subagents)                                                                     | `SKILL-phase-1-analyze.md` | `analysis-complete` |
| 2. Write (Change Specification + apply edits)                                                                                                       | `SKILL-phase-2-write.md`   | `write-complete`    |
| 3. Review (editorial + factual + cross-doc always; reader if user-facing/mixed; audit-completeness if deep audit mode — all dispatched in parallel) | `SKILL-phase-3-review.md`  | `review-complete`   |
...
- `critic-audit.md` — dispatched in Phase 3 (§3b Critic 5) for **deep audit mode only**, with the double-blind input scope defined in HARD RULE 2.
...
- [ ] `ambiguity-scan.sh` passes for each touched public, prescriptive, planning, or executable task file; advisory internal-log findings are reported.
