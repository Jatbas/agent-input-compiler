...
describe("compile-handler", () => {
  ...
  describe("model resolution integration", () => {
    ...
    it("fallback_intent_used_when_mcp_omitted_default_and_prior_non_general_exists", async () => {
      const tmpDir = fs.mkdtempSync(path.join(os.homedir(), "aic-compile-test-"));
      try {
        const { scope } = createScopeWithRealDb(tmpDir);
        scope.db
          ...
          .run(
            toUUIDv7("018f0000-0000-7000-8000-0000000000aa"),
        ...
        await handler(
          {
            intent: MCP_INTENT_OMITTED_DEFAULT,
            projectRoot: tmpDir,
            modelId: null,
            configPath: null,
            conversationId: "conv-mcp-default-fallback",
          },
          undefined,
        );
        expect(capturedRequests).toHaveLength(1);
