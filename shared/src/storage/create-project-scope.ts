...
export function createProjectScope(
  ...
  // always reconcile so projectId is registered in projects and FK constraints are satisfied
  const projectId: ProjectId = reconcileProjectId(
    projectRoot,
    db,
    clock,
    idGenerator,
  ...
  const compilationLogStore = new SqliteCompilationLogStore(projectId, db);
  const sessionTracker = new SqliteSessionStore(db);
  const fileTransformStore = new SqliteFileTransformStore(projectId, db, clock);
  return {
    db,
    clock,
    idGenerator,
