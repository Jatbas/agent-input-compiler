...
export function deduplicateImportsInText(content: string): string {
...
export function splitLeadingImportsAndBody(content: string): {
  readonly importLines: readonly string[];
  readonly body: string;
} {
  const lines = content.split("\n");
  const firstNonImport = lines.findIndex((line) => parseImportLine(line) === null);
  const importLines = firstNonImport < 0 ? lines : lines.slice(0, firstNonImport);
  const body = firstNonImport < 0 ? "" : lines.slice(firstNonImport).join("\n");
